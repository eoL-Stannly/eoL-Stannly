...
**A decentralized options desk with institutional liquidity providers on the other side. Any asset listed on Aevo, any strike, any expiry up to three months, and zero exchange fees.**
...
**•**  Industry estimates put only around a quarter of altcoin options volume on exchanges. The rest is negotiated OTC. This is that market, on-chain.
...
That's the gap Aevo OTC fills, and it's a bigger gap than it looks. Only a minority of altcoin options volume clears on exchanges at all. Everything else is negotiated privately, over chat, with settlement risk attached and no price you can verify.
...
An OTC option is a position on the Aevo L2 like any other, so it sits in the same account as your perpetual futures. If you're long ETH perps and you buy downside protection through OTC at a strike and expiry the listed grid doesn't offer, that protection nets against the perp for margin purposes in the normal way.

The case where this matters most is hedging something the listed book can't cover: a large position in an altcoin perp, or protection that needs to run to a specific date. Same downside protection logic as the built-in modes on the main exchange, applied where the exchange's own grid runs out.
...
## **Related reading**
  ...
  - *Aevo staking explained: tiers, epochs, the lottery and every reward* — options volume counts towards the year-end distribution
