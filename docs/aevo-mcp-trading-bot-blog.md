...
**TL;DR:** Aevo just shipped an **exchange MCP server** and an **Aevo Trading Bot**, giving AI agents native access to perpetuals and options trading — multi-leg strategies, portfolio Greeks, and pre-trade risk checks included. Connect any MCP-compatible AI agent directly to the Aevo exchange, or skip setup entirely and trade from a chat window on Telegram.

## Why an exchange MCP server, and why now

MCP servers for trading are proliferating. In crypto, integrations from CoinGecko, 0x-based aggregators, and Uniswap already give AI agents access to market data and spot swaps. TradFi platforms have shipped MCP servers with broader capabilities, including options. But across all of DeFi, none of these integrations can construct a multi-leg position, calculate portfolio Greeks, or run pre-trade risk validation — they stop exactly where the complexity starts.

That's exactly where Aevo has always operated. Our mission, since day one, has been to make sophisticated instruments accessible to a wider audience. AI agents are the next step in that arc: traders can now say *"build me a delta-neutral position on ETH"* or *"set up an iron condor with defined risk,"* and have an agent reason through the strategy, validate the risk, and execute it — all through Aevo's exchange MCP integration. The complexity becomes something you can delegate, while you stay focused on the outcome you want.

Today we're launching two new products that make this real:

- **The [Aevo MCP Server](https://github.com/ribbon-finance/aevo-mcp)** — an open-source **exchange MCP** integration that gives any AI agent full access to the Aevo exchange through 45+ structured tools, covering everything from market analysis to multi-leg options execution.
- **The [Aevo Trading Bot](https://t.me/Aevo_Trading_Bot)** on Telegram — the same capability, in a chat window, powered by Claude, ChatGPT, and other LLM backends, for traders who want AI-assisted trading without any setup.
...
The **Aevo MCP Server** is the first DeFi-native exchange MCP built for derivatives complexity — perpetuals and options — not just spot trading.

## 45+ tools for the full trading lifecycle

The server exposes over 45 tools organized across five categories that cover everything a trading agent needs:
...
- **Order Execution** — limit orders, bracket orders with entry + stop-loss + take-profit, and batch operations; not just single-shot trades but structured execution plans.
...
The server also ships with built-in prompts — `trade_plan`, `risk_checklist`, and `onboarding_plan` — that give your agent structured workflows instead of leaving it to figure out sequencing on its own.
...
- **Self-hosted via Docker:** clone the [repo](https://github.com/ribbon-finance/aevo-mcp), add your credentials, `docker compose up`.
...
There's also a companion [Aevo Trading Skills repo](https://github.com/ribbon-finance/aevo-trading-skills) that provides structured guidance for agents: instrument naming conventions, risk guardrails, workflow recipes, and an options strategy reference — like a trader's playbook that the AI can internalize.

## Bringing it to Telegram: the Aevo Trading Bot

Not everyone lives in a code editor — millions of crypto traders spend their days in Telegram, monitoring channels, discussing positions, reacting to news. So we built a **Trading Bot** that brings agentic trading directly into that environment.

The **[Aevo Trading Bot](https://t.me/Aevo_Trading_Bot)** connects to the same Aevo exchange MCP infrastructure and supports multiple LLM backends, including Claude, ChatGPT, and others. You can interact with it in natural language: ask it to analyze a market, check your positions, or walk you through a trade plan. It's the same 45+ tool capability set, accessible from a chat window — no code required.
...
The [Aevo Trading Bot](https://t.me/Aevo_Trading_Bot) is a Telegram bot built on the same Aevo MCP infrastructure. It lets you trade and analyze markets on Aevo using natural language, with support for Claude, ChatGPT, and other LLM backends — no local setup required.

**Do I need to know how to code to use the Trading Bot?**
No. The Telegram Trading Bot works entirely through chat. The self-hosted or hosted MCP server is aimed at developers who want to wire an agent (Claude Desktop, Claude Code, Cursor, Windsurf, OpenClaw, etc.) directly into Aevo.
